...
Prior to running an actual test, you should warm the database server up to ensure that data is loaded into memory. To do so, simply execute the Replay tool from the command-line using the warmup parameter.

```
SqlReplay.Console.exe run C:\capture\output\replay0.txt "dbo.usp_MyProcToInclude,dbo.usp_MyOtherProcToInclude" 30 "Server=MYSERVER;Database=MYDATABASE;User Id=user;Password=password;Max Pool Size=1000;Connection Timeout=60;"
```

The file path is the path to a binary data file generated by the prepping process.

Next is a comma separated list of stored procedures, which should include their schema(s). These stored procedures will be the only ones included in the warm up run. If you leave this as an empty string to, nothing will get run.

Next is the approximate duration in minutes that you want the warmup run to last. If you provide a number less than 1, nothing will get run.
...
```
SqlReplay.Console.exe run "C:\capture\output\replay0.txt" "dbo.usp_MyProcToExclude,dbo.usp_MyOtherProcToExclude" "Server=MYSERVER;Database=MYDATABASE;User Id=user;Password=password;Max Pool Size=1000;Connection Timeout=60;"
```
